...
void	CKinematicsAnimated::LL_BoneMatrixBuild	( CBoneInstance &bi, const Matrix4x4 *parent, const SKeyTable	&keys )
{
	// Blend them together
	...
	bi.mTransform.Multiply43(RES, *parent);
#ifdef DEBUG
		if(!check_scale(RES))
		{
...
void CKinematicsAnimated::BuildBoneMatrix(const CBoneData* bd, CBoneInstance &bi, const Matrix4x4 *parent, u8 channel_mask /*= (1<<0)*/)
{
	SKeyTable	keys;
