...
void CDrawUtilities::DrawFlag(const Fvector& p, float heading, float height, float sz, float sz_fl, u32 clr, BOOL bDrawEntity) {
	...
	if (bDrawEntity) {
		...
		FVF::L*	pVF = (FVF::L*)Stream->Lock(6, vs_L->vb_stride, vBase); sz *= 0.8f;
		pVF->set(p.x, p.y + height, p.z, clr); pVF++;
		pVF->set(p.x + rx * sz, p.y + height, p.z + rz * sz, clr); pVF++; sz *= 0.5f;
		pVF->set(p.x, p.y + height * (1.f - sz_fl * .5f), p.z, clr); pVF++;
		pVF->set(p.x + rx * sz*0.6f, p.y + height * (1.f - sz_fl * .5f), p.z + rz * sz*0.75f, clr); pVF++;
		pVF->set(p.x, p.y + height * (1.f - sz_fl), p.z, clr); pVF++;
		pVF->set(p.x + rx * sz, p.y + height * (1.f - sz_fl), p.z + rz * sz, clr); pVF++;
		Stream->Unlock(6, vs_L->vb_stride);
		// and Render it as line list
		DU_DRAW_DP(D3DPT_LINELIST, vs_L, vBase, 3);
	}
		...
		// fill VB
		FVF::L*	pVF = (FVF::L*)Stream->Lock(6, vs_L->vb_stride, vBase);
		pVF->set(p.x, p.y + height * (1.f - sz_fl), p.z, clr); pVF++;
		pVF->set(p.x, p.y + height, p.z, clr); pVF++;
		pVF->set(p.x + _sin(heading)*sz, ((pVF - 2)->p.y + (pVF - 1)->p.y) / 2, p.z + _cos(heading)*sz, clr); pVF++;
		pVF->set(*(pVF - 3)); pVF++;
		pVF->set(*(pVF - 2)); pVF++;
		pVF->set(*(pVF - 4)); pVF++;
		Stream->Unlock(6, vs_L->vb_stride);
...
void CDrawUtilities::DrawAABB(const Fvector& p0, const Fvector& p1, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire)
{
    ...
    R = DirectX::XMMatrixScaling			(_abs(p1.x-p0.x),_abs(p1.y-p0.y),_abs(p1.z-p0.z));
    R.TranslateOver(C);
	RCache.set_xform_world(R);
	DrawIdentBox	(bSolid,bWire,clr_s,clr_w);
}
...
{
	Matrix4x4 B;
	B = DirectX::XMMatrixScaling(radius, radius, radius);
	B.TranslateOver(center);
	B.Multiply43(B, parent);
...
{
	Matrix4x4 mScale;
	mScale = DirectX::XMMatrixScaling(2.f*radius, 2.f*radius, height);
	...
	Matrix4x4         	mR = DirectX::XMMatrixIdentity();
	mR.x = L_right;
	mR.y = L_up;
	mR.z = L_dir;
	mR.w = center;
	...
	// final xform
	Matrix4x4 xf;			xf.Multiply(mScale, mR);
	xf.Multiply43(xf, parent);
...
void CDrawUtilities::DrawObjectAxis(const Matrix4x4& T, float sz, BOOL sel)
{
    ...
    Fvector c,r,n,d;
	float w	= T.w[0]* mFullTransform.x[3] + T.w[1]* mFullTransform.y[3] + T.w[2]* mFullTransform.z[3] +  mFullTransform.w[3];
    if (w<0) return; // culling

	float s = w*sz;

								mFullTransform.Transform(c,T.w);
    r.mul(T.x,s); r.add(T.w); 	mFullTransform.Transform(r);
    n.mul(T.y,s); n.add(T.w); 	mFullTransform.Transform(n);
    d.mul(T.z,s); d.add(T.w); 	mFullTransform.Transform(d);
	c.x = (float)iFloor(_x2real(c.x)); c.y = (float)iFloor(_y2real(-c.y));
...
void CDrawUtilities::OutText(const Fvector& pos, LPCSTR text, u32 color, u32 shadow_color)
{
	...
	float w	= pos.x* mFullTransform.x[3] + pos.y* mFullTransform.y[3] + pos.z* mFullTransform.z[3] +  mFullTransform.w[3];
	if (w>=0)
	{
