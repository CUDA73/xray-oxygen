...
EFC_Visible	CFrustum::testSAABB(Fvector& c, float r, const float* mM, u32& test_mask) const
{
	...
	for (int i = 0; i < p_count; i++, bit <<= 1)
	{
		if (test_mask&bit)
		{
			...
			{
				EFC_Visible	rVis = AABB_OverlapPlane(planes[i], mM);
				if (fcvFully == rVis)	test_mask &= ~bit;					// fully - no need to test this plane
				else if (fcvNone == rVis) 
				{ 
					test_mask = 0; 
					return fcvNone; 
				}	// none - return
		...
		for (u32 j=0; j<src->size()-1; j++)
		{
			...
			if (negative(cls[j]))
			{
				...
				if (positive(cls[j+1]))
				{
					...
					Dvect.sub((*src)[j+1],(*src)[j]);
					denum = P.n.dotproduct(Dvect);
					if (denum!=0) {
						t = -cls[j]/denum; //VERIFY(t<=1.f && t>=0);
						dest->last().mad((*src)[j], Dvect, t);
						dest->inc();
					}
				}
			} else {
