...
{
	if (!a_taget_object || !a_taget_object->ObjectPPhysicsShell() || !a_taget_object->ObjectPPhysicsShell()->isActive() ||
		a_taget_object->IsInventoryItem() || !a_character || !a_character->b_exist || !a_character->PhysicsRefObject() ||
...

{
	...
	m_taget_element = m_taget_object->ObjectPPhysicsShell()->NearestToPoint(m_capture_bone->mTransform.w, cb);
	if (!m_taget_element)
		return;
	Init();
}
...
void CPHCapture::Init()
{
	...
	capture_bone_position.set(m_capture_bone->mTransform.w);
