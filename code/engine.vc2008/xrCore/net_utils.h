...
public:
    ...
    IC void w_s64(const s64 a) {
        w(&a, sizeof(s64));
    } // qword (8b)

    IC void w_u32(const u32 a) {
        w(&a, sizeof(u32));
    } // dword (4b)

    IC void w_s32(const s32 a) {
        w(&a, sizeof(s32));
    } // dword (4b)

    IC void w_u16(const u16 a) {
        w(&a, sizeof(u16));
    } // word (2b)

    IC void w_s16(const s16 a) {
        w(&a, sizeof(s16));
    } // word (2b)

    IC void w_u8(const u8 a) {
        w(&a, sizeof(u8));
    } // byte (1b)

    IC void w_s8(const s8 a) {
        w(&a, sizeof(s8));
    } // byte (1b)
